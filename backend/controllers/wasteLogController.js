...
exports.createWasteLog = async (req, res) => {
  const { category, weightKg, disposalDate, managerId } = req.body;
  try {
...
exports.getWasteLogs = async (req, res) => {
  try {
    const logs = await WasteLog.find().populate("managerId", "name");
    ...
    const totalRescuedWeight = completedListings.reduce((sum, item) => sum + item.weightKg, 0);
