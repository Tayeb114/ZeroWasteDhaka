...
export default function ActiveClaimNavigation() {
  ...
  const fetchActiveClaim = async () => {
    try {
      ...
      if (res.ok) {
        // Find if this volunteer has any active claimed listing
        const claimed = data.find(
          (item) => item.status === "claimed" && item.claimedBy && item.claimedBy._id === volunteerId
        );
        setActiveClaim(claimed || null);
        if (claimed) {
          setConfirmed(false);
        }
      }
    } catch (err) {
      console.error("Error fetching active claim:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/listings/${activeClaim._id}/complete`, {
        method: "PUT",
      });
      if (res.ok) {
        setConfirmed(true);
        ...
        <main className="p-6 lg:p-8 flex-1 flex flex-col">
          ...
          {loading ? (
            <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">Loading active claim details...</p>
            </div>
          ) : !activeClaim ? (
            <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
              <p className="text-sm text-gray-500 font-semibold">No active claimed listings found.</p>
              <p className="text-xs text-gray-400 mt-1">Visit the Browse Food Feed to claim today's leftovers.</p>
            </div>
          ) : (
              ...
              <div className="lg:col-span-5 bg-white rounded-2xl border border-gray-200 p-6 space-y-6">
                <div>
                  ...
                  <h2 className="font-display text-xl font-semibold text-gray-900 mt-3.5">
                    Claim #{activeClaim._id.slice(-6).toUpperCase()}
                  </h2>
                </div>
                ...
                <div className="rounded-xl border border-gray-150 bg-gray-50/50 p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900 mt-0.5">
                      {activeClaim.title} &bull; {activeClaim.weightKg} kg
                    </p>
                  </div>
                </div>
                  ...
                  <div className="relative pl-8 space-y-8">
                    ...
                    <div className="relative flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {confirmed ? "Delivered & logged +100 points to both profiles" : "Pending provider signature at pickup site"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start justify-between gap-3 p-4 rounded-xl border border-gray-150 bg-gray-50/50">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-sm font-semibold text-gray-900 mt-0.5">
                          {activeClaim.postedBy ? activeClaim.postedBy.name : "Star Restaurant"}
                        </p>
                        <p className="text-xs text-gray-500 mt-0.5">{activeClaim.location || "Dhanmondi, Dhaka"}</p>
                      </div>
                    </div>
                    <a
                  ...
                  <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-850 mt-1 leading-relaxed">
                        Please enter through the back kitchen entrance. Ask the shift supervisor for ZeroWaste claim ID #{activeClaim._id.slice(-6).toUpperCase()}.
                      </p>
                    </div>
                  </div>
                </div>
